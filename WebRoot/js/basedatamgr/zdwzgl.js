			...
			success : function(result) {
				if (null==result || ""==result) {
					return;
						...
						zhandianSelect.append("<option value=''>请选择地区</option>");
						mapZhandianSelect.append("<option value=''>请选择地区</option>");
						for(var zd in zhandianData){
							var option = "<option value='"+zhandianData[zd].dqid+"'>"+zhandianData[zd].dqmc+"</option>"
							...
							if($(this).val()){
					          ...
					          //  map.panBy(point);
					           ...
					           // local.getResults();
								...
								//更新站点
								$.ajax({
									url:getRootPath() + "zd_list_getByDiQuId.shtm",
									type : "POST",
									data :  '{"dqid":'+$(this).val()+'}',
									contentType : "application/json; charset=utf-8",
									datatype : "json",
									success : function(result) {
										if (null==result || ""==result) {
											...
											for(var sz in zbssDataList){
												...
												var sDiv = "<div objId='"+zbssDataList[sz].zdbaseid+"' mapX='"+mapX+"' mapY='"+mapY+"' style='border-bottom:1px solid #ccc;' >"+zbssDataList[sz].zdmc+"</div>"
									...
									success : function(result) {
										if (null==result || ""==result) {
											...
											for(var sz in zbssDataList){
												...
												if(zbStatus == "1"){
													var dragDiv = $("<div objId='"+zbssDataList[sz].zdbaseid+"' style='text-align:center;width:100px;height:25px;background:#fafafa;border:1px solid #ccc;'>"+zbssDataList[sz].zdmc+"	</div>");
													...
													var sDiv = "<div objId='"+zbssDataList[sz].zdbaseid+"' style='border-bottom:1px solid #ccc;' class='over' >"+zbssDataList[sz].zdmc+"</div>"
													sourceDiv.append(sDiv);
													
												}else{
													var sDiv = "<div objId='"+zbssDataList[sz].zdbaseid+"' style='border-bottom:1px solid #ccc;' >"+zbssDataList[sz].zdmc+"</div>"
													sourceDiv.append(sDiv);
												}
											}
											bindSourceClick();
			...
			$.ajax({
				...
				success : function(result) {
					if (null==result || ""==result) {
						...
						layer.alert('站点地图位置保存成功！',1);
					}
				}
			});
		}else{
			layer.alert("请选择一个站点",8);
		}
...
function refresh(){
	...
	$.ajax({
	url : getRootPath() + "dispatcher.shtm?mark=26",
	type : "POST",
	contentType : "application/json; charset=utf-8",
