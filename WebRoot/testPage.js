...
Page.prototype.view = function(){
    ...
    return '<div name="laypage'+ laypage.v +'" class="laypage_main laypageskin_'+ (conf.skin ? (function(skin){
        ...
        return conf.skip 
        ? '<span class="laypage_total"><label>\u5230\u7b2c</label><input type="number" min="1" max="'+conf.pages+'" onkeyup="this.value=this.value.replace(/\\D/, \'\');this.value=this.value>'+conf.pages+'?'+conf.pages+':this.value" class="laypage_skip"><label>\u9875</label>'
...
Page.prototype.render = function(){
    ...
    if(conf.hash){
        location.hash = '!laypage_'+ conf.item +'='+ conf.curr;
    }
};
